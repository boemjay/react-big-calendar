...
function getSlotMetrics(_ref) {
  ...
  return {
    ...
    getRange: function getRange(rangeStart, rangeEnd, ignoreMin, ignoreMax) {
      ...
      var renderedInColumnCell = dates.lte(
        rangeEnd,
    ...
    getRanges: function getRanges(rangeStart, rangeEnd, ignoreMin, ignoreMax) {
      ...
      if (!ignoreMax) rangeEnd = dates.min(end, dates.max(start, rangeEnd))
      var events = []
      var spannedDays = dates.range(rangeStart, rangeEnd, 'day').length // returns the date for "d", starting at 00:00:00
      ...
      for (var i = 0; i < spannedDays; i++) {
        var localRangeStart = dayStartDate(dates.add(rangeStart, 0, 'day'))
        var localRangeEnd = dates.add(rangeEnd, -i, 'day')
        var rangeStartMin = 0
        var rangeEndMin = positionFromDate(localRangeEnd)
        var renderedInLastColumnCell = dates.lte(
          localRangeEnd,
          dates.add(end, -step, 'minutes')
        )
        var top =
