...
function getSlotMetrics$1(_ref) {
  ...
  return {
    ...
    getRange: function getRange(rangeStart, rangeEnd, ignoreMin, ignoreMax) {
      ...
      var rangeEndMin = positionFromDate(rangeEnd) // step is measured in minutes

      var renderedInColumnCell = lte(rangeEnd, add(end, -step, 'minutes'))
      var top =
     ...
     */
    getRanges: function getRanges(rangeStart, rangeEnd, ignoreMin, ignoreMax) {
      if (!ignoreMin) rangeStart = min(end, max(start, rangeStart))
      if (!ignoreMax) rangeEnd = min(end, max(start, rangeEnd))
      var events = []
      var spannedDays = range(rangeStart, rangeEnd, 'day').length // returns the date for "d", starting at 00:00:00

      function dayStartDate(d) {
        return new Date(d.getFullYear(), d.getMonth(), d.getDate(), 0, 0, 0, 0)
      }

      for (var i = 0; i < spannedDays; i++) {
        var localRangeStart = dayStartDate(add(rangeStart, 0, 'day'))
        var localRangeEnd = add(rangeEnd, -i, 'day')
        var rangeStartMin = 0
        var rangeEndMin = positionFromDate(localRangeEnd)
        var renderedInLastColumnCell = lte(
          localRangeEnd,
          add(end, -step, 'minutes')
        )
        var top =
        ...
        events.push({
          ...
          height: (rangeEndMin / (step * numSlots)) * 100 - top,
          start: 0,
          startDate: add(localRangeStart, -1, 'seconds'),
...
var WeekContext = React.createContext(undefined)
